...
from tests.utils import get_with_wrong_id, validate_valid_files, upload_and_get_job_id, upload_file
...
async def test_mark_jobs_as_read(client: AsyncClient, fixture_validate_file_point):
    response = await client.put(f"{settings.API_V2_STR}/job/read", json=[fixture_validate_file_point])
    assert response.status_code == 200
...
#     job_id = await upload_and_get_job_id(client, "point", "valid_large.geojson")
