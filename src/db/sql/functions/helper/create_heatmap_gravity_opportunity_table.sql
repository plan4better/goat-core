...
CREATE OR REPLACE FUNCTION basic.create_heatmap_gravity_opportunity_table(
    input_table text, max_traveltime smallint, sensitivity int, potential_column text,
...
BEGIN
    ...
    EXECUTE format(
        'INSERT INTO %s 
        SELECT id, h3_lat_lng_to_cell(geom::point, 10) AS h3_index, %s AS max_traveltime, %s AS sensitivity,
            %s AS potential, basic.to_short_h3_3(h3_lat_lng_to_cell(geom::point, 3)::bigint) AS h3_3
        FROM (SELECT * FROM %s %s) input_table;', result_table_name, max_traveltime, sensitivity, potential_column, input_table, where_filter 
    );
